...
@section('subtitle', 'Update menu item details')

@section('content')
<div class="max-w-2xl">
    <form action="{{ route('admin.cyber.menu.update', $menuItem) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PATCH')
        ...
        <div class="card p-6 space-y-6">
            ...
            @if($menuItem->image)
                <div>
                    ...
                    <div class="inline-block p-4 bg-[#2d2d2d] border border-[#333] rounded-xl shadow-lg">
                        <img src="{{ asset('storage/' . $menuItem->image) }}" alt="{{ $menuItem->name }}" class="w-64 h-64 object-cover rounded-lg border border-[#444]">
                    </div>
                    <div class="mt-2">
                        <p class="text-xs text-[#6b6b6b]">Current image path:</p>
                        <p class="text-xs text-[#a0a0a0] font-mono break-all mt-1">{{ $menuItem->image }}</p>
                    </div>
                </div>
            @endif
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                    class="w-full px-4 py-3 bg-[#2d2d2d] border border-[#333] rounded-lg text-white placeholder-[#6b6b6b] focus:border-[#00d4aa] focus:ring-1 focus:ring-[#00d4aa] transition-colors">{{ old('description', $menuItem->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="space-y-3">
                    <label class="flex items-center space-x-3 p-3 bg-[#2d2d2d] rounded-lg cursor-pointer hover:bg-[#333] transition-colors">
                        <input type="checkbox" name="available_all_slots" value="1" {{ old('available_all_slots', $menuItem->available_all_slots) ? 'checked' : '' }}
                    ...
                    <div id="meal_slot_container" class="{{ old('available_all_slots', $menuItem->available_all_slots) ? 'hidden' : '' }}">
                        <select name="meal_slot_id" id="meal_slot_id"
                            ...
                            @foreach($mealSlots as $slot)
                                <option value="{{ $slot->id }}" {{ old('meal_slot_id', $menuItem->meal_slot_id) == $slot->id ? 'selected' : '' }}>
                                    {{ $slot->display_name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-[#6b6b6b]">Leave empty to keep current image. Max 2MB.</p>
                @error('image')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
                ...
                <div id="imagePreviewContainer" class="mt-4 hidden">
                    <label class="block text-sm font-medium text-white mb-2">New Image Preview</label>
                    <div class="inline-block p-4 bg-[#2d2d2d] border border-[#333] rounded-xl shadow-lg">
                        <img id="imagePreview" src="" alt="Preview" class="w-64 h-64 object-cover rounded-lg border border-[#444]">
                    </div>
                    <div class="mt-2">
                        <p class="text-xs text-[#6b6b6b]">New file path:</p>
