...
<div class="mb-8 animate-slide-in">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-4xl font-bold bg-gradient-to-r from-orange-500 via-pink-500 via-purple-500 via-blue-500 to-cyan-500 bg-clip-text text-transparent mb-2">Edit Category</h1>
            <p class="text-slate-600 dark:text-slate-400">Update category information</p>
        </div>
...
<div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
    ...
    <form action="{{ route('admin.menu.categories.update', $category) }}" method="POST" class="p-6">
        @csrf
        @method('PATCH')
        ...
        <div class="mb-6">
            ...
            <div class="px-4 py-3 border-2 border-slate-300 dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-700/50 text-slate-600 dark:text-slate-400">
                {{ $category->type === 'food' ? 'Food Category' : 'Kitchen Product Category' }}
            </div>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Category type cannot be changed after creation</p>
        </div>
