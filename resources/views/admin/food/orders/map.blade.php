...
@section('title', 'Food Orders Map')
@section('subtitle', 'View all active food orders on map')
...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div class="flex gap-2">
            ...
            <button onclick="copyMapUrl()" class="px-4 py-2 bg-[#ff6b35] hover:bg-[#e55a2b] text-white text-sm font-bold rounded-lg transition-colors">
                Copy Map URL
            </button>
        </div>
    </div>
                ...
                @foreach($orders as $order)
                    <div class="p-3 bg-[#2d2d2d] rounded-lg hover:bg-[#333] transition-colors cursor-pointer order-card" 
                        ...
                        <div class="flex items-start justify-between mb-2">
                            <div class="flex-1">
                                <a href="{{ $order['url'] }}" class="text-sm font-medium text-[#ff6b35] hover:underline">
                                    {{ $order['order_number'] }}
                                </a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Create markers for each order (all orange for Food orders)
            ...
            .bindPopup(`
                <div style="min-width: 220px;">
                    ...
                    <div style="margin-top: 8px; display: flex; gap: 4px;">
                        <a href="${order.url}" style="padding: 4px 8px; background: #ff6b35; color: white; text-decoration: none; border-radius: 4px; font-size: 11px; font-weight: bold;">View</a>
...
function highlightOrderCard(orderId) {
    document.querySelectorAll('.order-card').forEach(card => {
        card.classList.remove('ring-2', 'ring-[#ff6b35]');
        if (parseInt(card.dataset.orderId) === orderId) {
            card.classList.add('ring-2', 'ring-[#ff6b35]');
        }
    });
}
