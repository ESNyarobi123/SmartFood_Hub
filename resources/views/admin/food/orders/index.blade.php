...
@section('subtitle', 'Manage custom orders')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-white">Food Orders</h1>
            <p class="text-sm text-[#a0a0a0] mt-1">Manage custom orders</p>
        </div>
        <a href="{{ route('admin.food.orders.map') }}" class="px-4 py-2 bg-[#ff6b35] hover:bg-[#e55a2b] text-white font-medium rounded-lg transition-colors text-sm inline-flex items-center gap-2">
    ...
    <!-- Filters -->
    <div class="card p-4">
        <form action="{{ route('admin.food.orders.index') }}" method="GET" class="flex flex-wrap gap-4">
            ...
            <input type="date" name="date" value="{{ request('date') }}"
                class="px-4 py-2 bg-[#2d2d2d] border border-[#333] rounded-lg text-white text-sm focus:border-[#ff6b35]">
            ...
            @if(request()->hasAny(['status', 'date']))
                <a href="{{ route('admin.food.orders.index') }}" class="px-4 py-2 bg-[#333] text-white font-medium rounded-lg hover:bg-[#444] transition-colors">
                    Clear
                </a>
            @endif
        </form>
                ...
                <tbody class="divide-y divide-[#333]">
                    @forelse($orders as $order)
                        <tr class="hover:bg-[#2d2d2d] transition-colors">
                            <td class="px-4 py-4">
                                <a href="{{ route('admin.food.orders.show', $order) }}" class="text-sm font-medium text-[#ff6b35] hover:underline">
                                    {{ $order->order_number }}
                                </a>
                            ...
                            <td class="px-4 py-4">
                                <a href="{{ route('admin.food.orders.show', $order) }}" class="p-2 hover:bg-[#333] rounded-lg transition-colors inline-block">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center text-[#6b6b6b]">
                                No orders found
                            </td>
                        </tr>
                    @endforelse
